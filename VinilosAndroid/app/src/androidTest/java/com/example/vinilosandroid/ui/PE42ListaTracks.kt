...
class PE42ListaTracks {
    ...
    fun pE42ListaTracks() {
        ...
        val textView1 = onView(
            allOf(
                withId(R.id.nombreTrack), withText("Decisiones"),
                withParent(withParent(IsInstanceOf.instanceOf(androidx.cardview.widget.CardView::class.java))),
                isDisplayed()
            )
        )
        textView1.check(matches(withText("Decisiones")))

        val textView2 = onView(
            allOf(
                withId(R.id.durationTrack), withText("5:05"),
                withParent(withParent(IsInstanceOf.instanceOf(androidx.cardview.widget.CardView::class.java))),
                isDisplayed()
            )
        )
        textView2.check(matches(withText("5:05")))
    }
